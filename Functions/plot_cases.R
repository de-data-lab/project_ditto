plot_cases <- function(case_data,selected_fips,comparison_fips,primary_metric) {
  
  primary_metric <- enquo(primary_metric)
  
  plot_data_selected_fips <- case_data %>% 
    filter(fips == selected_fips) %>% 
    mutate(label = glue::glue("<b>County:</b> {full_county_name}<br>",
                              "<b>Week:</b> {week}<br>",
                              "<b>New Cases per 100k People:</b> {cases_per}<br>",
  ...
  p <- ggplot() +
    geom_line(data = d,aes(x = week, y = !!primary_metric, group = fips,text = label),color = "grey",size = 0.5,alpha = .5) +
    geom_line(data = plot_data_selected_fips,aes(x = week, y = !!primary_metric, group = fips,text = label),color = CR_red(),size = 1) +
    theme_minimal() +
