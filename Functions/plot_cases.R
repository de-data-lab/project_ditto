plot_cases <- function(case_data,selected_fips,comparison_fips) {
  ...
  plot_data_selected_fips <- case_data %>% 
    ...
    mutate(label = glue::glue("<b>County:</b> {full_county_name}<br>",
                              "<b>Week:</b> {week}<br>",
                              "<b>New Cases per Capita:</b> {cases_per}<br>",
  ...
  p <- ggplot() +
    geom_line(data = d,aes(x = week, y = cases_per, group = fips,text = label),color = "grey",size = 0.5,alpha = .5) +
    geom_line(data = plot_data_selected_fips,aes(x = week, y = cases_per, group = fips,text = label),color = CR_red(),size = 1) +
    theme_minimal() +
