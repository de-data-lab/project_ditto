...
data_aggregated %>% 
  ...
  AzureStor::storage_save_rds(container,file = "project_ditto/saved_rds/data_aggregated.RDS")

#Single row per fips, sparkline included
library(sparkline)
AzureStor::storage_load_rds(container,"project_ditto/saved_rds/data_aggregated.RDS") %>% 
  group_by(fips) %>% 
  summarize(
    sparkline = spk_chr(
      cases_per, type ="line",
  ...
  AzureStor::storage_save_rds(container,file = "project_ditto/saved_rds/data_sparklines.RDS")
